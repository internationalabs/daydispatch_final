...
<div class="card">
    <div class="card-header">
        <h5 class="card-title mb-0">Logistic Shippers</h5>
    </div>
    <div class="card-body">
        <div class="table-responsive dataTables_wrapper me-0 d-flex">
            <div class="col-lg-10 col-md-6 col-sm-12">
                <table>
                    ...
                    <tr>
                        ...
                        <form action="{{ route('Admin.Logistic.Shippers') }}">
                            <td>
                                <select class="form-control ml-1" id="Search_Criteria" name="Search_Criteria">
                                    <option value="company_name" @if($Search_Criteria == "company_name") selected @endif>Company Name</option>
                                    <option value="city" @if($Search_Criteria == "city") selected @endif>State / City</option>
                                    <option value="phone" @if($Search_Criteria == "phone") selected @endif>Phone</option>
                                    <option value="Mc_Number" @if($Search_Criteria == "Mc_Number") selected @endif>Mc Number</option>
                                    <option value="Company_USDot" @if($Search_Criteria == "Company_USDot") selected @endif>Company USDot</option>
                                </select>
                            </td>
                            <td>
                                <input class="form-control" id="Search_Value" placeholder="Enter Search Value" value="{{ $Search_Value }}"
            ...
            <div class="col-lg-2 col-md-6 col-sm-12">
                <a class="btn btn-success add-btn btn-block btn-sm" data-bs-toggle="modal" href="#AdNewCarriers"><i
                        class="ri-add-line align-bottom me-1"></i> Add New Logistic Shippers
                </a>
            </div>
        </div>
        <table id="example" class="table table-bordered dt-responsive nowrap table-striped align-middle text-center"
            ...
            @foreach ($Lisiting as $List)
                <tr>
                    ...
                    <td>
                        <button class="btn btn-primary" onclick="document.location.href = 'tel:{{ $List->phone }}'">{{ Str::mask($List->phone, '*', 0, 12) }}</button>
                    </td>
                    <td>
                        {{ $List->Mc_Number }}
                    </td>
                    ...
                    <td>
                        @if(count($List->history($List->id)) > 0)
                            <button class="btn btn-info get-history" data-bs-toggle="modal" data-bs-target="#showModal" id="">
                                View History
                                <input type="hidden" class="company_name" name="company_name" value="{{ $List->company_name }}">
                                <input type="hidden" class="company_id" name="company_id" value="{{ $List->id }}">
                            </button>
                        @endif
                    </td>
                </tr>
            ...
            <div class="card mt-4">
                <div class="card-body p-4">
                    <div class="text-center mt-2">
                        <h5 class="text-primary">Add New Logistic Shipper</h5>
                        <!--<p class="text-muted">Get your free Day Dispatch account now</p>-->
                    </div>
                    <div class="p-2 mt-4">
                        <form action="{{ route('Admin.LogShipper.Store') }}" method="POST" class="needs-validation"
                            novalidate>
                            @csrf
...
<script>
    $(".get-history").click(function () {
            ...
            $.ajax({
                url: '{{ route('Admin.History.Logistic.Shippers') }}',
                type: 'GET',
                data: {
                    'CompanyID': CompanyID
                },
