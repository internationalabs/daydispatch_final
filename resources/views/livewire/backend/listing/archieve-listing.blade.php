                ...
                <div class="col-lg-6 col-md-6 col-sm-12">
                    <table>
                        ...
                        <tr>
                            ...
                            <td>
                                <input class="form-control" id="Search_Value" placeholder="Enter Search Value" type="search"
                                    name="Search_Value">
                            </td>
                        </tr>
                        </tbody>
                ...
                <tbody>
                    @foreach ($Lisiting as $List)
                        <tr class="card-row" data-status="active">
                            <td>
                                <input type="checkbox" name="selected_ids[]" value="{{ $List->all_listing->id }}" class="row-checkbox">
                            </td>
                            <td>
                                <span style="font-size: x-large;"><strong>{{ $List->all_listing->Ref_ID }}</strong></span><br>
                            ...
                            <td>
                                ...
                                @foreach ($List->all_listing->vehicles as $vehcile)
                                    @php
                                        $vehicleText = "{$vehcile->Vehcile_Year} {$vehcile->Vehcile_Make} {$vehcile->Vehcile_Model}";
                                        $trimmedText = Str::words($vehicleText, 4, '...');
                                    @endphp
                            
                                    <span style="font-size: large;">
                                        <a class="font-weight-bold text-dark"
                                           href="https://www.google.com/search?q={{ urlencode($vehicleText) }}"
                                           target="_blank"
                                           data-bs-toggle="tooltip"
                                           data-bs-placement="top"
                                           title="{{ $vehicleText }}"> <!-- Tooltip Content -->
                            ...
                            @endif
                                ...
                                @if (count($List->all_listing->heavy_equipments) === 1)
                                    @foreach ($List->all_listing->heavy_equipments as $vehcile)
                                        <span style="font-size: large;"><a class="font-weight-bold text-dark"
                                                ...
                                                {{ $vehcile->Equipment_Model }}</a></span>
                                        <br>
                                ...
                                @if (count($List->all_listing->heavy_equipments) > 1)
                                    <a href="javascript:void(0)" tabindex="0" class="" data-toggle="popover"
                                        ...
                                        @if (!empty($vehcile->Trailer_Type))
                                            {{ $vehcile->Trailer_Type }} <br>
                                        @endif @endforeach"
                                        data-html="true">vehicles ({{ count($List->all_listing->heavy_equipments) }})
                                    </a>
                                @endif
                                ...
                                <div class="dropdown mt-3">
                                    <a href="javascript:void(0)" tabindex="0"
                                        ...
                                        @if ($List->all_listing->additional_info && $List->all_listing->additional_info->Additional_Terms)
                            ...
                            <td>
                                @if ($List->all_listing->paymentinfo !== null && isset($List->all_listing->paymentinfo->Price_Pay_Carrier))
                                    ...
                                    @endif
                                @else
                                    <strong>Job Price:</strong><span> ${{ $List->all_listing->paymentinfo->Price_Pay_Carrier ?? 0 }}
                                    </span><br>
                                @endif

                                @if ($List->all_listing->paymentinfo !== null && isset($List->all_listing->paymentinfo->COD_Amount))
                                    @if ($List->all_listing->paymentinfo->COD_Amount === '0')
                                    {{-- <strong>
                                                 Pay On Pickup OR Delivery not decided yet
                                             </strong>
                                             <br />
                                             <strong>
                                                 Pay On Delivery:
                                ...
                                @else
                                    <strong>
                                        @if ($List->all_listing->paymentinfo && $List->all_listing->paymentinfo->COD_Location_Amount == 'PickUp')
                                            Pay on Pickup:
                                        @else
                                            Pay on Delivery:
                                        @endif
                                    ...
                                    <span>${{ $List->all_listing->paymentinfo ? $List->all_listing->paymentinfo->COD_Amount : '0' }}</span>
                                    <br />
                                @endif
                                ...
                                {{-- <strong>Price to Pay:</strong><span>${{ $List->all_listing->paymentinfo->COD_Amount }}</span><br>
                                    <strong>COD/COP:</strong>{{ $List->all_listing->paymentinfo->COD_Method_Mode }} <br> --}}
                                @if (!empty($List->all_listing->request_broker->Offer_Price) && $List->all_listing->request_broker->Offer_Price !== 0)
                                    ...
                                    {{-- <strong>Ask Price:</strong>${{ $List->all_listing->request_broker->Offer_Price }} --}}
                                    <br>
                                @endif
                                ...
                                <strong>Price per
                                    Mile:</strong>
                                    @if ($List->all_listing->paymentinfo && $List->all_listing->routes)
                            ...
                            <td>
                                ...
                                {{ $List->all_listing->pickup_delivery_info->Delivery_Date }} <br>
                                <strong>Archived Date:</strong><br>{{ $List->all_listing->updated_at }}
                            </td>
                            <td>
                                @if ($List->all_listing->user_id === $currentUser->id)
                                    <a class="btn btn-outline-primary mb-2 w-100 d-block"
                                        href="{{ route('User.Listing.Delete', ['List_ID' => $List->all_listing->id]) }}">Delete
                                        Listing</a>
                                    ...
                                    <a class="btn btn-outline-primary mb-2 w-100 d-block"
                                        href="{{ route('User.Listing.Restore', ['List_ID' => $List->all_listing->id]) }}"
                                        role="button">Restore
                                        Listing</a>
                                @else
                                    <a class="btn btn-outline-primary mb-2 w-100 d-block"
