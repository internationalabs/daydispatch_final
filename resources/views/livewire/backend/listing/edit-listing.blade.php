...
<div class="card user-settings-box mb-30">
    <div class="card-body">
        <form class="was-validated" method="POST" action="{{ route('User.Listing.Update') }}"
            enctype="multipart/form-data">
            @csrf
            <h3><i class='bx bx-file'></i> Edit Listing</h3>
...
<script>
    window.onload = function() {
        ...
        var pickupDate = new Date($('#pdatate1').val()); // Get pickup date
        var deliveryDate = new Date($('.deliver-date').val()); // Get delivery date

        if (pickupDate < new Date()) {
            var njDate = new Date(new Date().toLocaleString("en-US", { timeZone: "America/New_York" }));

            // Extract month, day, and year
            var month = String(njDate.getMonth() + 1).padStart(2, '0'); // Months are 0-based, so add 1
            var day = String(njDate.getDate()).padStart(2, '0');
            var year = njDate.getFullYear();
        ...
        $('#pdatate1, .deliver-date').change(function() {
            ...
            // Convert to New Jersey timezone if needed
            ...
            if (pickupDateNJ > deliveryDateNJ) {
                ...
                $('#pdatate1').val(''); // Reset pickup date field
                return;
            }
...
{{-- <script>
    $(document).ready(function() {
        var BalanceAmount = '{{ $Lisiting->paymentinfo->Balance_Amount }}';
        var BalanceAmount = parseInt(BalanceAmount);

        console.log('BalanceAmount',  BalanceAmount);

        if (BalanceAmount !== 0) {
            $('.balance-inputs').show();
            $('#BalPaymentInfoInput').show();
            $(".balance-inputs select").prop('required', true);
        } else {
            $('.balance-inputs').hide();
            $('#BalPaymentInfoInput').hide();
