    ...
    <div class="card">
        <div class="card-body">
            <div class="table-responsive">
                ...
                <form method="POST" action="{{ route('bulk.action.quote') }}" id="bulk-action-form">
                    @csrf
                    <div class="d-flex align-items-center mb-3">
                        ...
                        <button type="button" id="submit-button" class="btn btn-outline-primary w-auto ms-2" style="display: none;" onclick="handleBulkAction()">Submit</button>
                    </div>
                <table
                    ...
                    <tbody>
                        ...
                        @endif --}}
                            <tr class="card-row" data-status="active">
                                <td>
                                    <input type="checkbox" name="selected_ids[]" value="{{ $List->id }}" class="row-checkbox">
                                </td>
                                <td>
                                    @php
                                        $isNewCustomer =
                                ...
                                <td>
                                    ...
                                    <div class="mb-2">
                                        <span class="fs-5">
                                            <strong>
                                                <a class="btn badge-primary text-white text-decoration-none fw-bold load-history"
                                ...
                                <td>
                                    @if ($List->paymentinfo)
                                        <strong>Job Price:</strong><span>
                                            ${{ $List->paymentinfo->Price_Pay_Carrier }}</span>
                                        <br>
                                        <strong>Book Price:</strong><span>
                                            ${{ $List->paymentinfo->Order_Booking_Price }}</span>
                                        <br>
                                ...
                                <td>
                                    ...
                                    @if ($List->user_id === $currentUser->id)
                                        ...
                                        @php
                                            $exist = App\Models\Listing\AllUserListing::where('order_id', $List->id)
                                                ->where('user_id', $List->user_id)
                                                ->first();
                                        @endphp
                                        @if (!$exist)
                                            <a class="btn btn-outline-primary mb-2 w-100 d-block text-nowrap"
                                                href="{{ route('MoveTo.Loardboard', ['List_ID' => $List->id, 'User_ID' => $List->user_id]) }}">Move To 
                                                Loardboard</a>
                                        @else<a class="btn btn-outline-primary mb-2 w-100 d-block text-nowrap disabled"
                                                href="{{ route('MoveTo.Loardboard', ['List_ID' => $List->id, 'User_ID' => $List->user_id]) }}">Already
                                                on Loadboard</a>
                                        @endif
                                        ...
                                        @if ($check != null)
                                            <a class="btn btn-outline-primary mb-2 w-100 d-block text-nowrap"
                                                href="{{ route('order.form.submitted', ['List_ID' => $List->id, 'User_ID' => $List->user_id]) }}">View
                                                Order
                                                Form</a>
                                        @else
                                            <a class="btn btn-outline-primary mb-2 w-100 d-block text-nowrap disabled"
                                        ...
                                        <a class="btn btn-outline-primary mb-2 w-100 d-block text-nowrap"
                                            href="#">Cancelled
                                            Order</a>
                                        {{-- <a class="btn btn-primary mb-2 w-100 d-block text-nowrap order-history" data-toggle="modal"
                                            data-target="#OrderHistory" href="javascript:void(0);">
                                            <input hidden type="text" id="hiddenListId" class="Listed-ID" value="{{ $List->id }}">Cancelled Order
                                            </a> --}}
                                    @endif
                                    {{-- </div>
                                </div> --}}
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <button type="button" class="close" data-id="{{ $List->id ?? '' }}" data-dismiss="modal" onclick="redirect(this)">
            ...
            <div class="modal-body user-settings-box" id="modal-body">
                ...
                <div id="form-section" style="display:none;">
                    <form id="order-status-form" action="{{ route('Order.Cancelled') }}" method="POST" class="was-validated" enctype="multipart/form-data">
                        @csrf
